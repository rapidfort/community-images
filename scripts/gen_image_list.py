...
def merge_yaml_files():
    ...
    Opens readme.yml for each image
    Creates an array with all readme.yml content
    ...
    with open(image_lst_path, "r") as stream:
        for image_path in stream.readlines():
            image_yml_path=os.path.join(script_path, "..", "community_images", image_path.rstrip(), "readme.yml")
            try:
