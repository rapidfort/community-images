...
class TestPrometheus():
  ...
  def test_s1(self, params):
    ...
    # self.driver.get("http://localhost:9090/graph?g0.expr=&g0.tab=1&g0.stacked=0&g0.show_exemplars=0&g0.range_input=1h")
    ...
    self.driver.execute_script("if(arguments[0].contentEditable === 'true') {arguments[0].innerText = '<div class=\"cm-line\">request_count_total</div>'}", element)
    # 7 | click | css=.execute-btn | 
    self.driver.find_element(By.CSS_SELECTOR, ".execute-btn").click()
    # search for the text on the page now
    assert "request_count_total" in self.driver.page_source
    # 8 | editContent | css=.cm-content | <div class="cm-line">request_latency_seconds_bucket</div>
    element = self.driver.find_element(By.CSS_SELECTOR, ".cm-content")
