...
CONTAINER_NAME=${PROJECT_NAME}-sidecar-1
echo "$CONTAINER_NAME"
# NAMESPACE=$(jq -r '.namespace_name' <<< "$JSON")
REPO_PATH=$(jq -r '.image_tag_details."k8s-sidecar-official".repo_path' <<< "$JSON")
TAG=$(jq -r '.image_tag_details."k8s-sidecar-official".tag' <<< "$JSON")
