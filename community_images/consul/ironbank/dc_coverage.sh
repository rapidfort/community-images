...
# exec into consul client(node3) and run coverage script
docker exec -i "${PROJECT_NAME}"-consul2-1 sh /opt/scripts/coverage_script.sh

# Query our service using HTTP API on consul-node1 via consul-node2
docker exec -i "${PROJECT_NAME}"-consul2-1 sh /opt/scripts/coverage_script.sh
